...
/**
 * @DOTO: jQuery Ajax通用分页插件 jquery.whatyPager-1.2.2.js
 * 		已测试支持jquery-1.9.1+
 * 		待优化点：
 ...
 * 			5、（v_1.2.4以优化）分页栏各个元素显示顺序实现配置控
 * 			6、（v_1.2.5以优化）分页栏元素样式优化，多提供几套默认主题皮肤，分页栏按钮样式采用默认模板同时，实现可佩控制，能通过配置使用图片或样式替换当前默认按钮
 * @version: version_1.2.2
 * @author: lixin
 */
(function($){
