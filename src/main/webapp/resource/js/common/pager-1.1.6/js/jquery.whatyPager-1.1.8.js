...
/**
 * @DOTO: 分页插件 jquery.whatyPager-1.1.8.js，未完善版，正在优化中，支持jquery-1.9.1以上版本
 * 			优化配置项，1、前后端数据交互可配置；如key=val中，key可根据后端情况进行配置
 * @version: version_1.1.8
 * @author: lixin
 */
(function($){
...
//			alert("url："+options.pagerUrl+"\r\ndata："+searchParam+"\r\npagerParam："+pagerParam);
	...
	$.fn.initWhatyPager.reloadPager = function(reloadOptions){
		...
		if(changeRowNum < 0) {					//删除
			...
			alert("reloadPager调用参数错误; \r\n如：$.fn.initWhatyPager.reloadPager({loadType: 'refresh|delete|add'})");
			return false;
		}
