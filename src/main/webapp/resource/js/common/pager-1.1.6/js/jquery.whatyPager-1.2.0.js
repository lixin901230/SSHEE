...
/**
 * @DOTO: jQuery Ajax通用分页插件 jquery.whatyPager-1.2.0.js
 * 		已测试支持jquery-1.9.1+
 * 		待优化点：
 * 			1、插件内分页核心数据统一化管理，优化使用$.data()缓存更新后的分页核心数据，取代pagerToolBar中的隐藏域
 * 			2、连续分页页码显示数配置控制，且连续分页页码为偶数时当前页码显示居中偏左还是偏右
 * 			3、连续分页前后配置控制是否显示多余页码省略按钮，显示，则点击该按钮重新定位当前页为该省略按钮的钱一页为当前页，并拉出后面的页码
