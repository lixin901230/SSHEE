...
/*$.fn.tree.defaults.loadFilter = function(data, parent) {
	...
	if (opt.parentField) {
		...
		for (i = 0, l = data.length; i < l; i++) {
			if(!data[i]['attributes'] && attrsField && attrsField.length > 0) {	// 后台数据中不存在attributes属性时判断前端是否有使用该方式来在前端处理
				var attr = {};
				if(attrsField instanceof Array) {
					for(var j=0; j<attrsField.length; j++) {
						attr[attrsField[j]] = data[i][attrsField[j]];
						//console.info(attr);
					}
				}
				data[i]['attributes'] = attr;
			}
			if (tmpMap[data[i][parentField]] && data[i][idFiled] != data[i][parentField]) {
				if (!tmpMap[data[i][parentField]]['children']){
					tmpMap[data[i][parentField]]['children'] = [];
